...
export function Focus({ insights }: { insights: Insights }) {
  ...
  // The advice is computed over the selected range, so the heading has to say which one.
  const period = RANGE_PERIOD_LABELS[insights.range]
    ...
    if (insights.recommendations.length === 0) {
      return (
        <EmptyCard
          ...
          message={`No recommendation stands out ${period} — keep going.`}
        />
      )
    }
