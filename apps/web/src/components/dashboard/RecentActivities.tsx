...
export function RecentActivities({ records }: { records: DailyRecord[] }) {
  ...
  const renderItems = () => {
    if (items.length === 0) {
      return <EmptyCard title="No workouts logged" message="Nothing recorded in this range." />
    }
        ...
        {items.map((item) => (
          <li key={item.id} className="flex items-center justify-between px-4 py-2.5 text-sm">
            ...
            <span className="figure text-ink-muted">
              {item.durationMin} min · {item.calories} kcal · {format(parseISO(item.date), 'MMM d')}
            </span>
          </li>
        ))}
      </ul>
